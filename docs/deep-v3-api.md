...
실제 모델 검사를 통과하는 [요청 JSON 예제](../apps/backend/tests/fixtures/deep_v3_contract_examples.json)와 최신 [OpenAPI](../apps/backend/openapi.json)를 제공한다. 값은 모두 가상 데이터·원 단위이며 예제 날짜는 2026년 9월 기준이다. 다른 시점에는 재원 기준일·시작월·납부일을 함께 변경한다.
