...
@pytest.mark.parametrize('share_finance,share_values', [(True, True), (False, True), (True, False)])
def test_reviewer_journey_over_real_https_and_mongo(share_finance, share_values):
    async def run():
        async with isolated_deep_database() as db:
            ...
            async with https_backend(db.name, uri, PASSWORDS) as (origin, context), AsyncExitStack() as stack:
                ...
                created = (await request(a, 'POST', '/api/v1/deep/sessions', expected=201, json={},
                                         headers={'Idempotency-Key': 'https-create'})).json()
                path = '/api/v1/deep/sessions/' + created['id']
                invitation = '/api/v1/deep/invitations/' + created['invitationCode'] + '/join'
                ...
                saved = (await request(a, 'PATCH', path + '/plan',
                                       json={'expectedVersion': plan['version'], 'plan': sample_plan()})).json()
                version = saved['version']
                for client in (a, b):
                    await request(client, 'POST', path + '/plan/confirm', json={'planVersion': version})
                submission = {'expectedRevision': 1, 'planVersion': version, 'consentVersion': 'deep-sharing-v1',
                ...
                agreement = (await request(a, 'POST', path + '/agreements', expected=201,
                                           json={'expectedRound': 1, 'text': '매월 공동 생활비를 함께 점검한다'})).json()
                for client in (a, b):
