...
function formatAmountValue(value: Amount["value"]): string {
...
function isKnownState(state: AmountState): boolean {
  return state === "known-exact" || state === "known-estimate" || state === "zero";
}

function precisionFromState(state: AmountState): Amount["precision"] {
  return state === "known-estimate" ? "estimate" : "exact";
}

interface AmountInteractionState {
  basePrecision: Amount["precision"];
  baseStatus: Amount["status"];
  baseValue: Amount["value"];
  rawValue: string;
  selectedState: AmountState;
}
...
function AmountEditor({ label, value, onChange, disabled = false }: {
  ...
  const [interactionState, setInteractionState] = useState<AmountInteractionState>(() => ({
    ...
    baseStatus: amount.status,
    baseValue: amount.value,
    rawValue: formatAmountValue(amount.value),
    selectedState: amountState(amount),
  }));
  const hasCurrentValue =
    interactionState.baseValue === amount.value &&
    interactionState.baseStatus === amount.status &&
    interactionState.basePrecision === amount.precision;
  const rawValue = hasCurrentValue ? interactionState.rawValue : formatAmountValue(amount.value);
  const selectedState = hasCurrentValue ? interactionState.selectedState : amountState(amount);

  const updateInteractionState = (next: Pick<AmountInteractionState, "rawValue" | "selectedState">) => {
    setInteractionState({
      basePrecision: amount.precision,
      baseStatus: amount.status,
      baseValue: amount.value,
  ...
  const changeValue = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const numericValue = parseAmountValue(raw);
    const nextState = numericValue === null
      ? selectedState
      : selectedState === "known-estimate" ? "known-estimate" : "known-exact";
    updateInteractionState({ rawValue: raw, selectedState: nextState });
    if (numericValue === null) return;

    onChange({ status: "known", precision: precisionFromState(nextState), value: numericValue });
  };

  const changeState = (nextState: AmountState) => {
    if (nextState === "zero") {
      updateInteractionState({ rawValue: "0", selectedState: nextState });
      onChange({ status: "known", precision: "exact", value: 0 });
      return;
    }

    if (nextState === "known-exact" || nextState === "known-estimate") {
      updateInteractionState({ rawValue, selectedState: nextState });
      const numericValue = parseAmountValue(rawValue);
      if (numericValue === null) return;
    ...
    updateInteractionState({ rawValue: "", selectedState: nextState });
    onChange(nextState === "unknown"
      ? { ...amount, status: "unknown", value: null }
      : { ...amount, status: "withheld", value: null });
  };
  ...
  return (
    <div className="space-y-2">
      ...
      <select
        ...
        disabled={disabled}
        onChange={(event) => changeState(event.currentTarget.value as AmountState)}
        value={selectedState}
      >
      ...
      {isKnownState(selectedState) && rawValue === "" ? <p className="text-sm text-red-700" role="alert">금액을 입력해 주세요.</p> : null}
    </div>
  );
}
    ...
    <form className="flex flex-col gap-8" onSubmit={(event) => event.preventDefault()}>
      <fieldset className="grid gap-5 sm:grid-cols-2" disabled={disabled}>
        ...
        <label className="space-y-2 text-sm font-semibold text-ink" htmlFor="plan-start-month">
          ...
          <input className={fieldClassName} id="plan-start-month" onChange={(event) => setField("startMonth", event.currentTarget.value)} type="month" value={plan.startMonth} />
        </label>
        <label className="space-y-2 text-sm font-semibold text-ink" htmlFor="plan-funding-as-of">
          <span className="block">재원 기준일</span>
          <input className={fieldClassName} id="plan-funding-as-of" onChange={(event) => setField("fundingAsOf", event.currentTarget.value)} type="date" value={plan.fundingAsOf} />
        </label>
        <label className="space-y-2 text-sm font-semibold text-ink sm:col-span-2" htmlFor="plan-housing-type">
          ...
          <select className={`${fieldClassName} font-normal`} id="plan-housing-type" onChange={(event) => setField("housingType", event.currentTarget.value as SharedPlanV3["housingType"])} value={plan.housingType}>
          ...
          {deadlines.map((deadline, index) => (
            <div className="grid gap-3 rounded-card border border-border-soft bg-card p-4 sm:grid-cols-[1fr_1fr_1fr_auto]" key={`${deadline.id}-${index}`}>
              <label className="space-y-2 text-sm font-semibold" htmlFor={`deadline-id-${index}`}>
                ...
                <input className={`${fieldClassName} font-normal`} disabled={disabled} id={`deadline-id-${index}`} onChange={(event) => onChange({ ...plan, fundingDeadlines: updateDeadline(deadlines, index, { id: event.currentTarget.value }) })} value={deadline.id} />
              </label>
              <label className="space-y-2 text-sm font-semibold" htmlFor={`deadline-date-${index}`}>
                <span className="block">날짜</span>
                <input className={`${fieldClassName} font-normal`} disabled={disabled} id={`deadline-date-${index}`} onChange={(event) => onChange({ ...plan, fundingDeadlines: updateDeadline(deadlines, index, { dueOn: event.currentTarget.value || null }) })} type="date" value={deadline.dueOn ?? ""} />
              </label>
      ...
      <section className="space-y-5" aria-labelledby="plan-loan-heading">
        ...
        {loan ? (
          <fieldset className="grid gap-5 rounded-card border border-border-soft bg-card p-5 sm:grid-cols-2" disabled={disabled}>
            ...
            <label className="space-y-2 text-sm font-semibold" htmlFor="plan-loan-id"><span className="block">대출 ID</span><input className={`${fieldClassName} font-normal`} id="plan-loan-id" onChange={(event) => setField("newHousingLoan", { ...loan, id: event.currentTarget.value })} value={loan.id} /></label>
            <label className="space-y-2 text-sm font-semibold" htmlFor="plan-loan-type"><span className="block">대출 종류</span><input className={`${fieldClassName} font-normal`} id="plan-loan-type" onChange={(event) => setField("newHousingLoan", { ...loan, type: event.currentTarget.value })} value={loan.type} /></label>
            <label className="space-y-2 text-sm font-semibold" htmlFor="plan-loan-repayment"><span className="block">상환 방식</span><select className={`${fieldClassName} font-normal`} id="plan-loan-repayment" onChange={(event) => setField("newHousingLoan", { ...loan, repaymentType: event.currentTarget.value as typeof loan.repaymentType })} value={loan.repaymentType}><option value="equalPayment">원리금균등</option><option value="equalPrincipal">원금균등</option><option value="bulletMaturity">만기일시</option><option value="unknown">모르겠어요</option></select></label>
            <label className="space-y-2 text-sm font-semibold" htmlFor="plan-loan-rate"><span className="block">금리</span><input className={`${fieldClassName} font-normal`} id="plan-loan-rate" onChange={(event) => setField("newHousingLoan", { ...loan, annualRate: event.currentTarget.value || null })} value={loan.annualRate ?? ""} /></label>
            ...
            <label className="space-y-2 text-sm font-semibold" htmlFor="plan-loan-available"><span className="block">새 대출금은 언제 사용할 수 있나요?</span><input className={`${fieldClassName} font-normal`} id="plan-loan-available" onChange={(event) => setField("newLoanAvailableOn", event.currentTarget.value || null)} type="date" value={plan.newLoanAvailableOn ?? ""} /></label>
            <label className="space-y-2 text-sm font-semibold" htmlFor="plan-loan-certainty"><span className="block">새 대출은 얼마나 확실한가요?</span><select className={`${fieldClassName} font-normal`} id="plan-loan-certainty" onChange={(event) => setField("newLoanCertainty", event.currentTarget.value as SharedPlanV3["newLoanCertainty"])} value={plan.newLoanCertainty}><option value="confirmed">확인됨</option><option value="expected">예상</option><option value="unknown">모름</option></select></label>
          </fieldset>
      ...
      <section className="space-y-5" aria-labelledby="plan-common-heading">
        ...
        <label className="space-y-2 text-sm font-semibold" htmlFor="plan-common-status"><span className="block">공동비 범위를 알고 있나요?</span><select className={`${fieldClassName} font-normal`} disabled={disabled} id="plan-common-status" onChange={(event) => setCommonExpensesStatus(event.currentTarget.value as SharedPlanV3["commonExpensesStatus"])} value={plan.commonExpensesStatus}><option value="known">알고 있어요</option><option value="unknown">모르겠어요</option><option value="withheld">공개하지 않을게요</option></select></label>
      ...
      <section className="space-y-5" aria-labelledby="plan-target-heading">
        ...
        {target ? <fieldset className="grid gap-5 rounded-card border border-border-soft bg-card p-5 sm:grid-cols-2" disabled={disabled}><legend className="sr-only">공동 목표 상세</legend><label className="space-y-2 text-sm font-semibold" htmlFor="plan-target-title"><span className="block">목표명</span><input className={`${fieldClassName} font-normal`} id="plan-target-title" onChange={(event) => setField("target", { ...target, title: event.currentTarget.value })} value={target.title} /></label><AmountEditor label="목표액" onChange={(amount) => setField("target", { ...target, amountWon: amount.value ?? 0 })} value={{ status: "known", precision: "exact", value: target.amountWon }} /><label className="space-y-2 text-sm font-semibold" htmlFor="plan-target-month"><span className="block">목표월</span><input className={`${fieldClassName} font-normal`} id="plan-target-month" onChange={(event) => setField("target", { ...target, targetMonth: event.currentTarget.value })} type="month" value={target.targetMonth} /></label></fieldset> : <p className="text-sm text-ink-muted">함께 모을 목표가 없으면 선택하지 않아도 돼요.</p>}
      </section>
    </form>
  );
}
