...
- 로그인 화면은 인증 없이 `GET /api/v1/auth/providers`를 먼저 호출해 `{ "kakao": true, "reviewer": false }`처럼 활성화된 로그인 방법만 노출한다. Deep 자체가 꺼진 배포에서는 이 경로도 404다.
- 변경 요청은 기존 `Origin` 검사를 유지한다. 세션 생성·초대 수락·합의안 생성은 `Idempotency-Key`도 필요하다. 합의안 생성은 같은 키와 같은 본문을 재전송하면 기존 결과를 반환하므로, 네트워크 재시도에는 같은 키를 유지하고 새 합의안에는 새 키를 만든다. 다른 출처 프론트의 브라우저 쿠키 연결은 기존 인증 계약에 따라 별도 검증한다.
