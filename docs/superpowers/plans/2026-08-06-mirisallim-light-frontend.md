...
- 산출물: 대기 중일 때 `partnerJoined`에 따라 주기를 나누는 `useSessionStatus(sessionId)`. 상대 미참여 상태에서는 2000ms, 참여가 감지된 뒤에는 1000ms 간격으로 폴링하며, 준비 완료·terminal 오류·언마운트에서는 멈춘다. 참여 전 화면은 초대 링크 재공유 상태를 보여 주므로 요청 예산이 정해지지 않은 현재는 기존 1000ms 대비 요청 빈도를 절반으로 줄이되 3초 공개 예산 아래에 남는 2000ms를 보수적으로 선택했다. 참여 감지 뒤 결과 공개를 좌우하는 주기는 1000ms로 유지한다.
